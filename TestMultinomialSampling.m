function TestMultinomialSampling(Word1, Word2)
    ...
    for m = 1:10^3
        ...
        for i = 1:length(K)
            ...
            %Get MLE (Approximate) Estimation
            MLE(m,i) = (M1*(2*n11+n21) + M2*(2*n11+n12) - sqrt(power(M1*(2*n11+n21) - M2*(2*n11 + n12),2) + 4*M1*M2*n12*n21))/(2*(2*n11+n12+n21));

            %Get Margin-Free Estimation
            MF(m,i) = (n11/(n11+n12+n21+n22))*N;
            
            %Get Theoretical MSE
            THR(m,i) = (N/n)/(1/N11 + 1/(M1-N11) + 1/(M2-N11) + 1/(N-M1-M2+N11));
    ...
    loglog(K, THR_mse,'g-.','linewidth',2); hold on; grid on;
